...
- **"Du bearbeitest:" select** — picks the SalesPerson currently being scheduled. This drives the +/− button behavior in cells (see below). **Note:** this dropdown and the driving-of-add/remove behavior is already implemented in the current codebase; only the rendering of a single button per cell (instead of two) is new.
...
- **+/− button logic:** there is exactly one button per cell; it adds or removes the SalesPerson currently selected in the "Du bearbeitest:" dropdown. If that person is already in the cell, the button shows `−` with `bad`-tinted styling; otherwise `+` with neutral styling. **No per-chip × button** — chips are display-only.
  - **Status:** today the codebase already drives add/remove via the "Du bearbeitest:" dropdown but renders **both** buttons (`+` and `−`) always. The change here is purely visual: render a single button whose icon depends on whether the editing person is currently booked in the slot.
- **State backgrounds:**
  ...
  - **Status:** the current codebase has **no** per-cell red and **no** column-wide red — conflicts surface only as a list above the grid. Adding column-wide tinting per this design is optional; the redesign master plan marks it as out of scope unless explicitly added back.
...
**Important:** `Sollstunden` does **not** belong here. Working hours are managed via Arbeitsverträge in the Employee detail page. *(Already correct in the current codebase — this is preserved intent, not a change.)*
...
> **Important — verify before changing.** Several of these points describe behavior that the current `shifty-dioxus` codebase **already implements**. They were preserved here as part of the design intent, but they are **not** changes requested against the current code. Each item below is annotated with its status. Don't redesign what's already there.

1. **No initials.** No `LB`, `MH` etc. anywhere. Avatar circles are color dots only. *(verify per-component)*
2. **No empty avatar in TopBar.** "Du bist [name]" is text only inside a pill.
3. **Red conflicts only on whole columns**, never on single cells or chips. **(✓ already correct in code — no per-cell red exists today; column-wide tinting is currently NOT shown either, conflicts surface only as a list above the grid. Adding column-wide tinting is optional and out of scope of the redesign master plan.)**
4. **Single +/− button per cell**, fixed top-right, drives the currently-edited SalesPerson. **(partial — the "Du bearbeitest:" dropdown and its driving of add/remove handlers is ALREADY in code; the only actual change is rendering ONE button (`+` or `−` depending on whether the editing person is in the slot) instead of two buttons that are always both shown.)**
5. **Sticky time column** in the shift grid for mobile horizontal scroll. **(✓ already implemented in `src/component/week_view.rs`.)**
6. **Sollstunden lives in Arbeitsverträge**, not in SalesPerson admin. **(✓ already correct in code — `sales_person_details.rs` has no `expected_hours` field; `expected_hours` lives in `employee_view.rs`.)**
7. **Benutzerverwaltung has two tabs**: SalesPerson and Benutzer. *(currently rendered as two side-by-side sections; visual change to tabs is part of redesign step `08`.)*

...
The TopBar SHALL render navigation links gated by user privileges. The link corresponding to the currently active route SHALL be visually highlighted using the accent-soft background.

#### Scenario: User with `sales` privilege sees Schichtplan
- **WHEN** the user has the `sales` privilege
- **THEN** the `Schichtplan` link SHALL be rendered

#### Scenario: User without any privilege sees no nav links
- **WHEN** the user has no privileges
- **THEN** no privilege-gated nav link SHALL be rendered
...
#### Scenario: Parameterised route highlights parent
- **WHEN** the current route is `Route::EmployeeDetails { … }`
- **THEN** the `Mitarbeiter` nav link SHALL render in the active highlighted state

#### Scenario: Privilege rules unchanged
- **WHEN** comparing privilege gating before and after this change
- **THEN** the visibility rules for each of the seven nav items SHALL be identical (no removals, no relaxations)
...
At viewports below 720 px, the TopBar SHALL collapse the navigation into a dropdown panel toggled by a burger button. The burger glyph SHALL switch between `☰` (closed) and `✕` (open). The panel SHALL auto-close when the route changes.
...
- **THEN** the dropdown panel SHALL appear below the bar containing all visible nav links AND the burger glyph SHALL change from `☰` to `✕`

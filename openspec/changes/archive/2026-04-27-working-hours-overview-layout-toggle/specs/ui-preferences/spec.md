# Capability: UI Preferences

## Purpose

Provide a small per-browser key/value store for non-critical UI preferences, backed by `localStorage`. Initial scope is the working-hours overview layout. Designed to host further per-browser UI prefs (sort orders, view modes, defaults) without each consumer hand-rolling its own `localStorage` access.

...
All four atom components SHALL be implemented in a dedicated `src/component/atoms/` module, separate from the legacy `base_components.rs`. The module SHALL re-export the public types from `src/component/mod.rs` so that consumers can import from `crate::component::{Btn, PersonChip, TupleRow, NavBtn}`.

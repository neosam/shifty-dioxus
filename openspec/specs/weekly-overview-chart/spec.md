# Weekly Overview Chart Specification

## Purpose

The weekly overview chart provides a visual representation of staffing data across an entire year on the weekly overview page. It renders an SVG stacked bar chart showing paid and volunteer hours per calendar week, overlaid with a required-hours reference line, so users can quickly identify seasonal staffing patterns, paid-vs-volunteer ratios, and weeks that miss the required-hours target without scanning 52 rows of tabular data.
## Requirements

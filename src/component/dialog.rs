...
///   tests of consumers (`use_media_query("(max-width: 720px)")` →
///   `Center`) remain deterministic.
/// - On WASM, the listener is registered via `add_event_listener_with_callback`
///   and is removed when the component unmounts (via the `Drop` impl on the
...
pub fn use_media_query(query: &'static str) -> Signal<bool> {
    ...
    // `signal` is a `Signal<bool>` (Copy) — return as-is.
    let _ = &mut signal;
    signal
}
